    ...
    internal partial class CommandsAnalyzer : ICollectSymbols, IGenerateCode
    {
        ...
        public void CollectSymbols(ProjectEnvironment environment)
        {
            ...
            foreach (var method in environment.MethodsWithAttributes)
            {
                ...
                if (method.TryGetCommandAttribute(environment.Compilation, _logger, out var commandAttribute))
                {
                ...
                if (method.TryGetFrontCommandAttribute(environment.Compilation, _logger, out var frontCommandAttribute))
                {
    ...
    internal class CommandMethodInfo : FrontCommandMethodInfoBase
    {
        ...
        public void Collect(ProjectEnvironment environment)
        {
            ...
            for (int i = 0; i < Symbol.Parameters.Length; i++)
            {
                ...
                if (parameter.TryGetArgumentAttribute(environment.Compilation, logger, out var argumentAttribute))
                {
                ...
                if (parameter.TryGetOptionAttribute(environment.Compilation, logger, out var optionAttribute))
                {
